...
class InferModel:
    ...
    def query_model(self, x_t):
        ...
        prediction_sub = []
        label_sub = []
        decoder_attention_t = torch.zeros(self.batch_size_t,1,dense_input,output_area_t)
        attention_sum_t = torch.zeros(self.batch_size_t,1,dense_input,output_area_t)
        decoder_attention_t_cat = []


        for i in range(self.maxlen):
            decoder_output, decoder_hidden_t, decoder_attention_t, attention_sum_t = self.attn_decoder1(decoder_input_t,
                                                                                            decoder_hidden_t,
                                                                                            output_highfeature_t,
                                                                                            output_area_t,
                                                                                            attention_sum_t,
                                                                                            decoder_attention_t,dense_input,self.batch_size_t,h_mask_t,w_mask_t,0)
